...
export const convertCmToLitres = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (!currentUser || !currentUser.id) {
    res.status(401).json({ message: 'Utilisateur non authentifie ou ID manquant.' });
    return;
  }
  ...
  try {
    ...
    if (error || !entry) {
      res.status(404).json({ message: `Aucune conversion trouvee pour ${cm} cm.` });
      return;
    }
...
export const getConversionHistory = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  const accessorUserIdForLog = accessorUser?.id || 'Systeme';
  ...
  try {
    ...
    if (error) {
      ...
      res.status(500).json({ message: 'Erreur serveur lors de la recuperation de l'historique global.', error: error.message });
      return;
    }
...
export const deleteConversionEntry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (!currentUser || !currentUser.id) {
    ...
    res.status(403).json({ message: 'Action non autorisee. Utilisateur non authentifie.' });
    return;
  }
  ...
  try {
    ...
    if (fetchError || !entry) {
      ...
      res.status(404).json({ message: 'Entree de conversion non trouvee.' });
      return;
    }
    ...
    if (entry.user_id !== currentUser.id && (!adminGoogleId || currentUser.id !== adminGoogleId)) {
      ...
      res.status(403).json({ message: 'Action non autorisee. Vous ne pouvez supprimer que vos propres entrees.' });
      return;
    }
    ...
    res.status(200).json({ message: 'Entree supprimee avec succes.' });
  } catch (error: any) {
...
export const bulkDeleteConversionEntries = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (!adminGoogleId) {
    ...
    res.status(500).json({ message: "Erreur de configuration serveur (privileges administrateur)." });
    return;
  }
  ...
  if (currentUser.id !== adminGoogleId) {
    ...
    res.status(403).json({ message: 'Action non autorisee. Privileges administrateur requis pour la suppression groupee.' });
    return;
  }

  if (!Array.isArray(ids) || ids.length === 0) {
    res.status(400).json({ message: 'Liste d'IDs requise pour la suppression groupee.' });
    return;
  }
  ...
  try {
    ...
    if (deleteError) {
      ...
      res.status(500).json({ message: 'Erreur serveur lors de la suppression groupee.', error: deleteError.message });
      return;
    }

    res.status(200).json({ message: `${ids.length} entree(s) supprimee(s) avec succes par l'administrateur.` });
  } catch (error: any) {
    console.error('BULK DELETE: Error during bulk delete operation:', error);
    res.status(500).json({ message: 'Erreur serveur lors de la suppression groupee.', error: error.message });
  }
};
